...
ws() {
	if (!isDefined(game["ws"])) {
		game["ws_connected"] = false;
		game["ws"] = websocket_connect("ws://localhost:8080/ws", "", ::onWebSocketConnect, ::onWebSocketMessage, ::onWebSocketClose, ::onWebSocketError);
	}
    ...
    while(true) {
        ...
        if (game["ws"] >= 0 && game["ws_connected"]) {
            websocket_sendText(game["ws"], "Hello from CoD2x GSC at " + i);
            i++;
        }

		if (i > 100000) {
			websocket_close(game["ws"]);
		}
    }
}

onWebSocketConnect() {
    println("Script: WebSocket #" + game["ws"] + " connected");
	game["ws_connected"] = true;
}
onWebSocketMessage(message) {
    println("Script: WebSocket #" + game["ws"] + " message received: " + message);
}
onWebSocketClose(byRemote) {
    println("Script: WebSocket #" + game["ws"] + " closed, byRemote: " + byRemote);
    
	// Server closed the connection, client will try to reconnect
	if (byRemote) {
		game["ws_connected"] = false;
	// Client closed the connection (via the websocket_close function), connection is now fully closed
	} else {
        game["ws"] = -1;
	}
}
onWebSocketError(error) {
    println("Script: WebSocket #" + game["ws"] + " error: " + error);
}
	...
	if (!isDefined(self.pers["matchinfo_ingame"]))
	{
		...
		if (matchIsActivated())
		{
			if (self matchPlayerIsAllowed() == false) {
				...
				iprint_to_team_players(self.name + "^7's team is " + self matchPlayerGetData("team_name"));
				iprint_to_team_players(self.name + "^7's name is " + self matchPlayerGetData("name"));
			}
		}
	}
