...
init()
{
	...
	if(game["firstInit"])
	{
		precacheString2("STRING_AUTO_SPECTATOR_KILLCAM_PROPOSAL_KEY_MULTIKILL", &"Press ^3[F]^7 to play ^2multikill ^7 of:");
		precacheString2("STRING_AUTO_SPECTATOR_KILLCAM_PROPOSAL_KEY_MULTIKILL_2", &"Press ^3[F]^7 to play ^22 kills ^7 of:");
		precacheString2("STRING_AUTO_SPECTATOR_KILLCAM_PROPOSAL_KEY_MULTIKILL_3", &"Press ^3[F]^7 to play ^23 kills ^7 of:");
		precacheString2("STRING_AUTO_SPECTATOR_KILLCAM_PROPOSAL_KEY_MULTIKILL_4", &"Press ^3[F]^7 to play ^24 kills ^7 of:");
		precacheString2("STRING_AUTO_SPECTATOR_KILLCAM_PROPOSAL_KEY_MULTIKILL_5", &"Press ^3[F]^7 to play ^25 kills ^7 of:");
		precacheString2("STRING_AUTO_SPECTATOR_KILLCAM_PROPOSAL_KEY_NOZOOM", &"Press ^3[F]^7 to play ^2noozom ^7 of:");
		precacheString2("STRING_AUTO_SPECTATOR_KILLCAM_PROPOSAL_KEY_BASH", &"Press ^3[F]^7 to play ^2bash ^7 of:");

		precacheString2("STRING_SPECTATINGSYSTEM_KILLCAM_REPLAY", &"Spectator killcam replay");
	}


	level.spectatingSystem_killcam_kills = [];
	level.spectatingSystem_killcam_proposingTime = 6;		// in seconds
	level.spectatingSystem_killcam_intenseSituation = false;
...
onConnected()
{
	self.spectatingSystem_killcam_indexes = [];
	self.spectatingSystem_killcam_proposingVisible = false;
...
onMenuResponse(menu, response)
{
	if (menu == game["menu_spectatingsystem"])
	{
		...
		else
		{
			...
			if (i > -1)
			{
				if (i < self.spectatingSystem_killcam_indexes.size)
					replayAction(self.spectatingSystem_killcam_indexes[i], true);
				else
...
onPlayerKilled(eInflictor, eAttacker, iDamage, sMeansOfDeath, sWeapon, vDir, sHitLoc, timeOffset, deathAnimDuration)
{
	...
	if (isDefined(eAttacker) && isPlayer(eAttacker) && eAttacker != self && save)
	{
		...
		killRecord.kills = 1;
		killRecord.fullyFollowedBySpectator = isDefined(level.spectatingSystem_player) && eAttacker == level.spectatingSystem_player;
		...
		for (i = level.spectatingSystem_killcam_kills.size-1; i >= 0; i--)
		{
			lastRecord = level.spectatingSystem_killcam_kills[i];
			...
			{
				killRecord.fullyFollowedBySpectator = killRecord.fullyFollowedBySpectator && level.spectatingSystem_killcam_kills[i].fullyFollowedBySpectator;
				killRecord.followedKillsBySpectator += level.spectatingSystem_killcam_kills[i].followedKillsBySpectator;
				killRecord.nozoom = killRecord.nozoom || level.spectatingSystem_killcam_kills[i].nozoom;
				killRecord.bash = killRecord.bash || level.spectatingSystem_killcam_kills[i].bash;
				killRecord.grenade = level.spectatingSystem_killcam_kills[i].grenade;	// first kill was grenade (need more time)
				killRecord.lastKill = killRecord.lastKill || level.spectatingSystem_killcam_kills[i].lastKill;
				killRecord.timeFirst = level.spectatingSystem_killcam_kills[i].timeFirst;
				killRecord.kills += level.spectatingSystem_killcam_kills[i].kills;

				level.spectatingSystem_killcam_kills[i].deleted = true;
		...
		// Add new
		id = level.spectatingSystem_killcam_kills.size;
		level.spectatingSystem_killcam_kills[level.spectatingSystem_killcam_kills.size] = killRecord;
...
{
	record = level.spectatingSystem_killcam_kills[killId];
...
potencialAutoKillcamForPlayer(killId)
{
	...
	for(;;)
	{
		...
		if (level.spectatingSystem_killcam_intenseSituation)
			continue;
		...
		if (!isDefined(level.spectatingSystem_player) || !isDefined(level.spectatingSystem_player.autospectator_visibleEnemy))
		...
		if (time_killer_no_enemy >= 3 && time_spectated_no_enemy >= 3 && !self.spectatingSystem_killcam_proposingVisible)
		{
			...
			while(self.spectatingSystem_killcam_proposingVisible)
			{
...
showKillcamProposingForPlayer(killId)
{
	...
	if (self.spectatingSystem_killcam_proposingVisible)
		return;

	self.spectatingSystem_killcam_proposingVisible = true;

	record = level.spectatingSystem_killcam_kills[killId];
	time = level.spectatingSystem_killcam_proposingTime;
	disableTime = 1.5;
	prematureEnd = .5;
	fadeOutTime = 0.25;
...
watchIntenseSituation()
{
	...
	for (;;)
	{
		...
		level.spectatingSystem_killcam_intenseSituation = (intenseSituationTime > 3);



		//if (level.spectatingSystem_killcam_intenseSituation != lastIntenseSituation)
		//	println("^3intenseSituation: " + level.spectatingSystem_killcam_intenseSituation);

		lastIntenseSituation = level.spectatingSystem_killcam_intenseSituation;
	}
}
...
HUD_SetRowsInMenu()
{
	...
	for (i = level.spectatingSystem_killcam_kills.size-1; i >= 0; i--)
	{
		record = level.spectatingSystem_killcam_kills[i];
		...
		self.spectatingSystem_killcam_indexes[row] = i;
