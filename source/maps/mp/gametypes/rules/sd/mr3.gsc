...
{
	game["leagueLogo"] = "";
	game["leagueString"] = &"S&D MR3 Mode";
	game["leagueStringOvertime"] = &"S&D MR3 Mode  ^3Overtime";
...
GetCvars()
{
	...
	arr = ruleCvarDefault(arr, "scr_sd_half_round", 3);	// Number of rounds when half-time starts. 0=ignored
	arr = ruleCvarDefault(arr, "scr_sd_half_score", 0);	// Number of score when half-time starts. 0=ignored
	arr = ruleCvarDefault(arr, "scr_sd_end_round", 6);		// Number of rounds when map ends. 0=ignored
	arr = ruleCvarDefault(arr, "scr_sd_end_score", 4);		// Number of score when map ends. 0=ignored

	// Round options
	arr = ruleCvarDefault(arr, "scr_sd_strat_time", 4);	// Time before round starts where players cannot move
	...
	// OT Rules
  	if (isDefined(game["overtime_active"]) && game["overtime_active"])
  	{
		...
		arr = ruleCvarDefault(arr, "scr_sd_end_score", 4);
    }
