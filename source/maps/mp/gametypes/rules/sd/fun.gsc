...
{
	game["leagueLogo"] = "";
	game["leagueString"] = &"S&D Fun mode";
...
GetCvars()
{
	...
	arr = ruleCvarDefault(arr, "scr_sd_roundlength", 1);	// Time length of each round (min)
	arr = ruleCvarDefault(arr, "scr_sd_end_time", 4);		// Time et the end of the round (after last player is killed) when you can finding weapons for the next round
	arr = ruleCvarDefault(arr, "scr_sd_count_draws", 0);	// If players are killed at same time - count this round (1) or play new round (0)?
	arr = ruleCvarDefault(arr, "scr_sd_round_report", 0);		// Print kill and damage stats at the end of the round

	// Bomb settings
	arr = ruleCvarDefault(arr, "scr_sd_bombtimer_show", 1);	// Show bombtimr stopwatch
	arr = ruleCvarDefault(arr, "scr_sd_bombtimer", 30);		// Time untill bomb explodes. (seconds)
	arr = ruleCvarDefault(arr, "scr_sd_PlantTime", 3);
	arr = ruleCvarDefault(arr, "scr_sd_DefuseTime", 4);
	arr = ruleCvarDefault(arr, "scr_sd_plant_points", 10);
	arr = ruleCvarDefault(arr, "scr_sd_defuse_points", 5);
	...
	*******************************/
	...
	arr = ruleCvarDefault(arr, "scr_regen_delay", 3000); 		//Time before regen starts
	...
	arr = ruleCvarDefault(arr, "scr_auto_deadchat", 0);					// Automaticly enable / disable deadchat
	...
	// Specific fun cvars
	arr = ruleCvarDefault(arr, "g_speed", 270);
	arr = ruleCvarDefault(arr, "g_gravity", 500);
