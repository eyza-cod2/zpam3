...
spawnPlayer()
{
	...
	if(isdefined(self.pers["weapon1"]) && isdefined(self.pers["weapon2"]))
	{
        ...
        // Give player spawned weapon if there is no weapon
		// If player switch from scope to shotgun, primary will be removed becuse its limited weapon, so give him selected weapon, no matter the secondary (so secondary is not set as primary in code below)
        if (weapon1 == "none")
            weapon1 = self.pers["weapon"];
