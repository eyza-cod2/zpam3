...
init()
{
	if(game["firstInit"])
	{
		...
		game["STRING_SPECTATINGSYSTEM_DAMAGEINFO_ON"] = 		"Hit info ^2On";
		game["STRING_SPECTATINGSYSTEM_DAMAGEINFO_OFF"] = 		"Hit info ^1Off";
	}
...
onConnected()
{
	if (!isDefined(self.pers["spectatingSystem_damageInfo"]))
		self.pers["spectatingSystem_damageInfo"] = true;
}


toggle()
{
	self.pers["spectatingSystem_damageInfo"] = !self.pers["spectatingSystem_damageInfo"];

	if (self.pers["spectatingSystem_damageInfo"])
		self iprintln(game["STRING_SPECTATINGSYSTEM_DAMAGEINFO_ON"]);
	else
		self iprintln(game["STRING_SPECTATINGSYSTEM_DAMAGEINFO_OFF"]);
}
...
*/
// Is called even if spectating system is disabled!
onPlayerDamaged(eInflictor, eAttacker, iDamage, iDFlags, sMeansOfDeath, sWeapon, vPoint, vDir, sHitLoc, psOffsetTime)
{
	...
	if (isDefined(eAttacker) && isPlayer(eAttacker) && self != eAttacker)
	{
		...
		forSpectators = level.spectatingSystem && isDefined(level.spectatingSystem_player) && eAttacker == level.spectatingSystem_player;
		...
		if (forPlayer || forSpectators)
		{
			...
			if (forSpectators)
			{
				...
				{
					player = players[i];
					if (player.pers["team"] == "streamer" && player.spectatingSystem_turnedOn && player.pers["spectatingSystem_damageInfo"] && !isDefined(player.killcam))
					{
	...
	// Show taken damage
	if (level.spectatingSystem && isDefined(level.spectatingSystem_player) && self == level.spectatingSystem_player && !level.in_readyup)
	{
		damage = iDamage;
...
{
	if (isDefined(eAttacker) && isPlayer(eAttacker) && self != eAttacker && isDefined(level.spectatingSystem_player) && eAttacker == level.spectatingSystem_player && !level.in_readyup)
	{
		// Show to all spectators
		players = getentarray("player", "classname");
		for(i = 0; i < players.size; i++)
...
showTakenDamageInfo(iDamage, sHitLoc, sMeansOfDeath)
{
	...
	self.hud_takendamageinfo.y = 130;
	self.hud_takendamageinfo.alpha = 1;
	self.hud_takendamageinfo_loc.y = 130;
