...
emptyName()
{
	...
	for(;;)
	{
		...
		hasBracesPattern = false;
		for (open_i = 0; open_i + 1 < self.name.size; open_i++)
		{
			if (self.name[open_i] == "[" && self.name[open_i + 1] == "{")
			{
				// Require at least one character inside: [{x}]
				for (close_i = open_i + 3; close_i + 1 < self.name.size; close_i++)
				{
					if (self.name[close_i] == "}" && self.name[close_i + 1] == "]")
					{
						hasBracesPattern = true;
						break;
					}
				}
		...
		if (hasBracesPattern)
		{
			...
			wait level.fps_multiplier * 1; // wait untill the name is trully renamed
		}
...
CodeCallback_PlayerDamage(eInflictor, eAttacker, iDamage, iDFlags, sMeansOfDeath, sWeapon, vPoint, vDir, sHitLoc, timeOffset)
{
	    ...
	    isDefined(sWeapon) && sWeapon == "shotgun_mp" && sMeansOfDeath == "MOD_PISTOL_BULLET")
	{
		...
		else if (dist < 800 && eAttacker.hitData[self_num].id == 1)
		{
			...
			if (level.debug_shotgun) eAttacker iprintln("^9Distance " + int(dist) + " | range-4 500-800 | linear " + iDamage + "hp damage");
			if (eyza_debug) printToEyza("### Consistent shotgun: attacker:"+eAttacker.name+" | victim:"+self.name+" | distance:" + int(dist) + " | hitLoc:" + sHitLoc + " | range-4 500-800 | linear damage:" + iDamage); // EYZA_DEBUG
