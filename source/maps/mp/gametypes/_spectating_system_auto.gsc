...
init()
{
	...
	level.spectatingSystem_auto_turnedOn = true;
	level.spectatingSystem_auto_lockedUntill = 0;
	level.spectatingSystem_auto_statusText = "STRING_AUTO_SPECTATING";
	level.spectatingSystem_auto_runThreads = false;
...
autoSpectatorDisable()
{
	...
	level.spectatingSystem_auto_turnedOn = false;

	level.spectatingSystem_auto_statusText = "STRING_AUTO_SPECTATING_OFF";
}

autoSpectatorToggle()
{
	if (!level.spectatingSystem_auto_turnedOn)
		autoSpectatorEnable();
	else
		autoSpectatorDisable();
}
...
HUD_loop()
{
	...
	if(!isdefined(self.spectatingSystem_HUD_autoSpectator_BG))
	{
		self.spectatingSystem_HUD_autoSpectator_BG = newClientHudElem2(self);
		self.spectatingSystem_HUD_autoSpectator_BG.archived = false;
		self.spectatingSystem_HUD_autoSpectator_BG.x = -40;
		self.spectatingSystem_HUD_autoSpectator_BG.y = 40;
		self.spectatingSystem_HUD_autoSpectator_BG.horzAlign = "center";
		self.spectatingSystem_HUD_autoSpectator_BG.vertAlign = "top";
		self.spectatingSystem_HUD_autoSpectator_BG.alpha = 0;
		self.spectatingSystem_HUD_autoSpectator_BG.color = (0, 0, 0);
		self.spectatingSystem_HUD_autoSpectator_BG setShader("white", 80, 10);
	}

	// Auto-spectating
	if(!isdefined(self.spectatingSystem_HUD_autoSpectator))
	{
		self.spectatingSystem_HUD_autoSpectator = newClientHudElem2(self);
		self.spectatingSystem_HUD_autoSpectator.horzAlign = "center";
		self.spectatingSystem_HUD_autoSpectator.vertAlign = "top";
		self.spectatingSystem_HUD_autoSpectator.alignX = "center";
		self.spectatingSystem_HUD_autoSpectator.alignY = "top";
		self.spectatingSystem_HUD_autoSpectator.x = 0;
		self.spectatingSystem_HUD_autoSpectator.y = 42;
		self.spectatingSystem_HUD_autoSpectator.archived = false;
		self.spectatingSystem_HUD_autoSpectator.font = "default";
		self.spectatingSystem_HUD_autoSpectator.alpha = 0;
		self.spectatingSystem_HUD_autoSpectator.sort = 2;
		self.spectatingSystem_HUD_autoSpectator.fontscale = 0.6;
	}
	...
	for (;;)
	{
		...
		if (!self.spectatingSystem_turnedOn || self.spectatingSystem_freeSpectating)
		{
			// Hide
			if (self.spectatingSystem_HUD_autoSpectator_BG.alpha != 0) 	self.spectatingSystem_HUD_autoSpectator_BG.alpha = 0;
			if (self.spectatingSystem_HUD_autoSpectator.alpha != 0) 	self.spectatingSystem_HUD_autoSpectator.alpha = 0;
		}
		else if (level.spectatingSystem_auto_turnedOn)
		{
			// Show
			if (self.spectatingSystem_HUD_autoSpectator_BG.alpha != 0.5) 	self.spectatingSystem_HUD_autoSpectator_BG.alpha = 0.5;
			if (self.spectatingSystem_HUD_autoSpectator.alpha != 1) 	self.spectatingSystem_HUD_autoSpectator.alpha = 1;

			canSwitch = ((gettime() - level.spectatingSystem_auto_lockedUntill) > 0)  &&  ((gettime() - level.spectatingSystem_lastChange) > 6000);

			color = (0, 0, 0); // black
			if (canSwitch) color = (0.03, 0.18, 0.1); // green

			if (self.spectatingSystem_HUD_autoSpectator_BG.color != color) 	self.spectatingSystem_HUD_autoSpectator_BG.color = color;
		}
		else
		{
			// No bg
		...
		if (level.spectatingSystem_auto_statusText != "" && level.spectatingSystem_auto_statusText != hud_text_last)
		{
			self.spectatingSystem_HUD_autoSpectator setText(game[level.spectatingSystem_auto_statusText]);
		}
		hud_text_last = level.spectatingSystem_auto_statusText;
	...
	if(isdefined(self.spectatingSystem_HUD_autoSpectator_BG))
	{
		self.spectatingSystem_HUD_autoSpectator_BG destroy2();
		self.spectatingSystem_HUD_autoSpectator_BG = undefined;
	}
	if(isdefined(self.spectatingSystem_HUD_autoSpectator))
	{
		self.spectatingSystem_HUD_autoSpectator destroy2();
		self.spectatingSystem_HUD_autoSpectator = undefined;
	}
}
...
printToAutoSpectators(text)
{
	...
	{
		player = players[i];
		if (player.pers["team"] == "streamer" && player.spectatingSystem_turnedOn && level.spectatingSystem_auto_turnedOn)
		{
...
playersCanSeeEachOthers()
{
	...
	{
		// Not enabled, wait
		if (!level.spectatingSystem_auto_runThreads)
		{
		...
		for(i = 0; i < players.size; i++)
		{
			...
			player.spectatingSystem_visibleEnemyInFuture = undefined;
			player.spectatingSystem_seenByEnemyInFuture = undefined;
		}
		...
		for(i = 0; i < players.size; i++)
		{
			...
			if ((player.pers["team"] == "allies" || player.pers["team"] == "axis") && (player.sessionstate == "playing"))
			{
				...
				{
					if (!isDefined(player.spectatingSystem_eyeWaypoint))
					{
						player.spectatingSystem_eyeWaypoint = spawn("script_origin",(0,0,0));
						player.spectatingSystem_eyeWaypoint thread maps\mp\gametypes\global\developer::showWaypoint();
					}
					if (isDefined(predictedPosition))
						player.spectatingSystem_eyeWaypoint.origin = predictedPosition;
					else
					{
						player.spectatingSystem_eyeWaypoint delete();
						player.spectatingSystem_eyeWaypoint = undefined;
					}
				}
						...
						if (angle_diff > -35 && angle_diff < 35)
						{
							...
							else if (isDefined(predictedPosition))
							{
								...
								if (headVisible)
								{
									...
									player.spectatingSystem_visibleEnemyInFuture = enemy;
									enemy.spectatingSystem_seenByEnemyInFuture = player;
								}
							}
						}
...
lockFor(time)
{
	level.spectatingSystem_auto_lockedUntill = gettime() + time; // wait some time untill next change

	if (level.spectatingSystem_auto_turnedOn)
		level.spectatingSystem_auto_statusText = "STRING_AUTO_SPECTATING_REASON_MANUAL";
}


autoFollowedPlayer()
{
	level.spectatingSystem_auto_lockedUntill = gettime() + 18000; // wait some time untill next change (also consider strat time)
	...
	for(;;)
	{
		...
		level.spectatingSystem_auto_runThreads = (autospectator_exists && players_exists);

		// No players, do nothing
		if (level.spectatingSystem_auto_runThreads == false)
			continue;
		...
		{
			wait level.fps_multiplier * 1;
			level.spectatingSystem_auto_lockedUntill = gettime() + 3000; // wait some time untill next change
			level.spectatingSystem_auto_statusText = "STRING_AUTO_SPECTATING_OFF";
			continue;
		}

		// Can switch after 6 second from last switch
		if ((gettime() - level.spectatingSystem_lastChange) < 6000)
		{
			continue;
		}
		...
		canSwitch = (gettime() - level.spectatingSystem_auto_lockedUntill) > 0;
		if (!canSwitch)
		{
			...
			if (isDefined(spectated_player.autospectator_visibleEnemy) || isDefined(spectated_player.autospectator_seenByEnemy))
			{
				...
				level.spectatingSystem_auto_lockedUntill = gettime() + 8000;
				level.spectatingSystem_auto_statusText = "STRING_AUTO_SPECTATING_REASON_VISIBLE_ENEMY";
				wait level.fps_multiplier * 1;
			}

			// Player has in future visible enemy on their screen or enemy is seeing this player or their screen
			// Extent time for this player
			if (isDefined(spectated_player.spectatingSystem_visibleEnemyInFuture) || isDefined(spectated_player.spectatingSystem_seenByEnemyInFuture))
			{
				if (level.debug_spectator) printToAutoSpectators("Extending time for this player (visisble enemy in future)");
				level.spectatingSystem_auto_lockedUntill = gettime() + 4000;
				level.spectatingSystem_auto_statusText = "STRING_AUTO_SPECTATING_REASON_VISIBLE_ENEMY_IN_FUTURE";
				wait level.fps_multiplier * 1;
			}
			...
			if (follow_team != "")
			{
				for(i = 0; i < players.size; i++)
				{
					...
					if (player.pers["team"] == follow_team && (player.sessionstate == "playing" || player.sessionstate == "dead"))
					{
						...
						level.spectatingSystem_auto_lockedUntill = gettime() + 1000; // wait some time untill next change
						level.spectatingSystem_auto_statusText = hud_text;
						break;
					}
				}
				continue;
		...
		if (level.gametype == "sd" && level.bombplanted && alliesAlive >= 1 && axisAlive >= 1)
		{
			if ((gettime() - level.bombtimerstart) < 3000) // if no action, follow player who actually planted the bomb
			{
				...
				{
					level maps\mp\gametypes\_spectating_system::followPlayer(level.planting_player);
					if (level.debug_spectator) printToAutoSpectators("Bomb planted, following planter");
					level.spectatingSystem_auto_lockedUntill = gettime() + 4000; // wait some time untill next change
					level.spectatingSystem_auto_statusText = "STRING_AUTO_SPECTATING_REASON_BOMB_PLANTED";
					continue;
				}
			}
			else if (game["allies_score"] != game["axis_score"])
			{
				...
				level.spectatingSystem_auto_statusText = "STRING_AUTO_SPECTATING_REASON_BOMB_SCORE";
			}
		}
		...
		if (level.gametype == "hq" && (level.DefendingRadioTeam == "allies" || level.DefendingRadioTeam == "axis"))
		{
			...
			level.spectatingSystem_auto_statusText = "STRING_AUTO_SPECTATING_REASON_RADIO_DEFEND";
		}
		...
		for(i = 0; i < players.size; i++)
		{
			...
			if ((player.pers["team"] == followed_team))
			{
				...
				// Player has visible enemy on their screen
				if (isDefined(player.spectatingSystem_visibleEnemyInFuture))
				{
					follow = true;
				...
				if ((gettime() - level.spectatingSystem_lastChange) > 16000)
				{
					...
					else if (isDefined(player.spectatingSystem_seenByEnemyInFuture))
					{
						follow = true;
