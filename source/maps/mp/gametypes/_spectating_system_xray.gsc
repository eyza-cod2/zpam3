...
init()
{
	...
	if(game["firstInit"])
	{
		...
		game["STRING_SPECTATINGSYSTEM_XRAY_ON"] = 		"XRAY ^2On";
		game["STRING_SPECTATINGSYSTEM_XRAY_OFF"] = 		"XRAY ^1Off";

		precacheString2("STRING_SPECTATINGSYSTEM_ARROW_LEFT", &"<");
		precacheString2("STRING_SPECTATINGSYSTEM_ARROW_RIGHT", &">");
	}
...
onConnected()
{
	if (!isDefined(self.pers["spectatingSystem_XRAY"]))
		self.pers["spectatingSystem_XRAY"] = false;

	self.spectatingSystem_XRAY_inFreeSpectating = false;
}


toggle()
{
	if (self.spectatingSystem_freeSpectating)
	{
		self.spectatingSystem_XRAY_inFreeSpectating = !self.spectatingSystem_XRAY_inFreeSpectating;
		self.pers["spectatingSystem_XRAY"] = self.spectatingSystem_XRAY_inFreeSpectating;
	}
	else
		self.pers["spectatingSystem_XRAY"] = !self.pers["spectatingSystem_XRAY"];

	if (self.pers["spectatingSystem_XRAY"])
		self iprintln(game["STRING_SPECTATINGSYSTEM_XRAY_ON"]);
	else
		self iprintln(game["STRING_SPECTATINGSYSTEM_XRAY_OFF"]);
}
...
{
	// Destroy all
	for(i = 0; i < self.spectatingSystem_XRAY_names.size; i++)
	{
		// If HUD is not defined, it means player disconnect and HUD object was deleted
		if (isDefined(self.spectatingSystem_XRAY_names[i]))
			self.spectatingSystem_XRAY_names[i] destroy2();
		if (isDefined(self.spectatingSystem_XRAY_images[i]))
			self.spectatingSystem_XRAY_images[i] destroy2();
	}
	self.spectatingSystem_XRAY_names = undefined;
	self.spectatingSystem_XRAY_images = undefined;
...
XRAY_Loop()
{
	...
	if (isDefined(self.spectatingSystem_XRAY_names))
		self XRAY_Destroy();
	self.spectatingSystem_XRAY_names = [];
	self.spectatingSystem_XRAY_images = [];

	self thread FollowCloseByPlayer();

	i_XRAY = 0;
	freeSpectating = self.spectatingSystem_freeSpectating;

	for(;;)
	{
		// Clear unused HUD elements
		for (i = self.spectatingSystem_XRAY_names.size - 1; i >= i_XRAY; i--)
		{
			// If HUD is not defined, it means player disconnect and HUD object was deleted
			if (isDefined(self.spectatingSystem_XRAY_names[i]))
				self.spectatingSystem_XRAY_names[i] destroy2();
			...
			self.spectatingSystem_XRAY_names[i] = undefined;
			self.spectatingSystem_XRAY_images[i] = undefined;
		...
		if (!isDefined(self.spectatingSystem_XRAY_names) || !isDefined(self.spectatingSystem_XRAY_images))
			break;
		...
		if (freeSpectating != self.spectatingSystem_freeSpectating && self.spectatingSystem_freeSpectating)
			self.spectatingSystem_XRAY_inFreeSpectating = true;
		freeSpectating = self.spectatingSystem_freeSpectating;

		// Get spectated player
		spectated_player = undefined;
		if (self.spectatingSystem_turnedOn && isDefined(level.spectatingSystem_player))
		...
		{
			// XRAY is turned off
			if (!self.spectatingSystem_XRAY_inFreeSpectating)
				continue;
		}
		else
		{
		...
		for(i = 0; i < players.size; i++)
		{
			...
			isNew = false;
			if (!isDefined(self.spectatingSystem_XRAY_names[i_XRAY]))
			{
				self.spectatingSystem_XRAY_names[i_XRAY] = addHUDClient(self, 10, 10, 1.2, (1,1,1), "center", "bottom", "subleft", "subtop");
				self.spectatingSystem_XRAY_names[i_XRAY].name = "name";
				self.spectatingSystem_XRAY_names[i_XRAY].archived = false;

				self.spectatingSystem_XRAY_images[i_XRAY] = addHUDClient(self, 10, 10, 1.2, (1,1,1), "center", "bottom", "subleft", "subtop");
				self.spectatingSystem_XRAY_images[i_XRAY].name = "layout";
				self.spectatingSystem_XRAY_images[i_XRAY].archived = false;
				self.spectatingSystem_XRAY_images[i_XRAY].sort = -1;
			...
			// Update HUD
			if (isNew || self.spectatingSystem_XRAY_names[i_XRAY].player != player)
			{
				self.spectatingSystem_XRAY_names[i_XRAY].player = player;
				self.spectatingSystem_XRAY_names[i_XRAY].offset = (0, 0, 25);
				self.spectatingSystem_XRAY_names[i_XRAY].tag = "head";
				self.spectatingSystem_XRAY_names[i_XRAY].string = "name"; // name | < | >
				self.spectatingSystem_XRAY_names[i_XRAY].fontscale = 0.8;
				self.spectatingSystem_XRAY_names[i_XRAY] SetPlayerNameString(player);
				self.spectatingSystem_XRAY_names[i_XRAY] thread SetPlayerWaypoint(self, player);
				self.spectatingSystem_XRAY_names[i_XRAY] thread hud_waypoint_animate(self, player);

				self.spectatingSystem_XRAY_images[i_XRAY].player = player;
				self.spectatingSystem_XRAY_images[i_XRAY].offset = (0, 0, -15);
				self.spectatingSystem_XRAY_images[i_XRAY] thread SetPlayerWaypoint(self, player);
				self.spectatingSystem_XRAY_images[i_XRAY] thread hud_waypoint_animate(self, player);
			}
...
FollowCloseByPlayer()
{
	...
	self.spectatingSystem_HUD_followPlayerName = addHUDClient(self, 0, 50, 1.2, (1,1,1), "center", "top", "center", "top");
	self.spectatingSystem_HUD_followPlayerName.alpha = 0;
	...
	for(;;)
	{
		...
		for(i = 0; i < self.spectatingSystem_XRAY_names.size; i++)
		{
			hud = self.spectatingSystem_XRAY_names[i];
		...
		if (isDefined(saved_player))
		{
			...
			{
				self.spectatingSystem_HUD_followPlayerName fadeOverTime(time);
				self.spectatingSystem_HUD_followPlayerName.alpha = 0;
			...
			if (!isDefined(saved_player_last) || saved_player_last != saved_player)
			{
				self.spectatingSystem_HUD_followPlayerName SetPlayerNameString(saved_player);
				self.spectatingSystem_HUD_followPlayerName fadeOverTime(time);
				self.spectatingSystem_HUD_followPlayerName.alpha = 1;
				wait level.fps_multiplier * time;
				if (!isDefined(self)) break; // in case player disconnects
			}
		}
		...
		self.spectatingSystem_playerToFollow = saved_player;
	}

	self.spectatingSystem_HUD_followPlayerName destroy2();
}
...
hud_waypoint_animate(spectator, player)
{
	...
	for (;;)
	{
		...
		if (!isDefined(spectator.killcam) && spectator.spectatingSystem_turnedOn && isDefined(level.spectatingSystem_player))
			spectated_player = level.spectatingSystem_player;
		...
		if (isAlive(player))
		{
			...
			if (self.name == "name")
			{
				...
				if (self.x < 40)
				{
					...
					{
						self.string = "<";
						self SetText(game["STRING_SPECTATINGSYSTEM_ARROW_LEFT"]);
						self.fontscale = 1.2;
					}
				}
				else if (self.x > 640 - 40)
				{
					...
					{
						self.string = ">";
						self SetText(game["STRING_SPECTATINGSYSTEM_ARROW_RIGHT"]);
						self.fontscale = 1.2;
					}
				}
				else
				...
				if (dist < 1500 || spectator.spectatingSystem_freeSpectating || self.string == "<" || self.string == ">")
					alpha = 1;
			}
			...
			if (self.name == "layout")
			{
				...
				if (!spectated_player isPlayerInSight(player) && !spectator.spectatingSystem_freeSpectating)
					alpha = 0.5;
			}

			// Hide if mouse is over
			if (!spectator.spectatingSystem_freeSpectating)
			{/*
