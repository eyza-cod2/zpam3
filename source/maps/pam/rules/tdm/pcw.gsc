...
{
    // Match Style
	setcvar("scr_tdm_timelimit", 15);		// Time limit. When halftime is enabled, its time limit per half. 0=disabled (minutes)
	setcvar("scr_tdm_half_score", 0);		// Number of score when half-time starts. Has no effect when halftime is disabled.
	setcvar("scr_tdm_end_score", 0);		// Number of score when map ends. 0=ignored

	// Halftime
	setcvar("scr_tdm_halftime", 1);			// Do halftime. When 1, scr_tdm_timelimit means time per half
  ...
  /*********
	...
	*********/
	...
	game["leagueLogo"] = "";
	game["leagueString"] = &"PCW Team Deathmatch";
}
