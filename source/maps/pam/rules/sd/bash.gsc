Rules()
{
	...
	setcvar("scr_sd_half_round", 0);	// Number of rounds when half-time starts. 0=ignored
	setcvar("scr_sd_half_score", 0);	// Number of score when half-time starts. 0=ignored
	setcvar("scr_sd_end_round", 0);		// Number of rounds when map ends. 0=ignored
	setcvar("scr_sd_end_score", 1);		// Number of score when map ends. 0=ignored

	// Round options
	setcvar("scr_sd_strat_time", 4);	// Time before round starts where players cannot move
	setcvar("scr_sd_roundlength", 0);	// Time length of each round (min)
	...
	setcvar("scr_sd_PlantTime", 0);
	setcvar("scr_sd_DefuseTime", 0);
	...
	*******************************/
	...
	setcvar("scr_timeouts_half", 0); 				// How many per side
	setcvar("scr_timeout_length", 0); 				// Length in minutes
	...
	*********/
	...
	setcvar("scr_boltaction_allow_drop", 0);
	setcvar("scr_sniper_allow_drop", 0);
	setcvar("scr_semiautomatic_allow_drop", 0);
	setcvar("scr_smg_allow_drop", 0);
	setcvar("scr_mg_allow_drop", 0);
	...
	setcvar("scr_allow_primary_drop", 0);
	...
	game["leagueLogo"] = "";
	game["leagueString"] = &"S&D Bash Mode";
}
