Rules()
{
	...
	setcvar("scr_sd_end_score", 0);		// Number of score when map ends. 0=ignored

	// Round options
	setcvar("scr_sd_strat_time", 3);	// Time before round starts where players cannot move
	setcvar("scr_sd_roundlength", 1);	// Time length of each round (min)
	setcvar("scr_sd_end_time", 5);		// Time et the end of the round (after last player is killed) when you can finding weapons for the next round
	...
	setcvar("scr_sd_bombtimer", 30);		// Time untill bomb explodes. (seconds)
	setcvar("scr_sd_PlantTime", 3);
	setcvar("scr_sd_DefuseTime", 4);
	setcvar("scr_sd_plant_points", 50);
	setcvar("scr_sd_defuse_points", 25);
	...
	*******************************/
	...
	setcvar("scr_regen_delay", 2500); 		//Time before regen starts
